...
# Start 3proxy with the generated config
# -f: foreground mode (don't daemonize, so logs go to stdout)
exec /usr/local/3proxy/bin/3proxy /usr/local/3proxy/conf/3proxy.cfg -f
